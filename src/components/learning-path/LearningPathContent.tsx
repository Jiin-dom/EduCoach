...
export function LearningPathContent({
    ...
    useEffect(() => {
        ...
        if (autoGeneratedTaskIds.current.has(nextQuizTask.task.id)) return

        autoGeneratedTaskIds.current.add(nextQuizTask.task.id)
        generateReview.mutate(
            {
                documentId: nextQuizTask.task.documentId,
                focusConceptIds: nextQuizTask.task.conceptIds,
                questionCount: Math.max(5, Math.min(12, nextQuizTask.task.conceptIds.length * 2)),
            },
            {
                onSuccess: () => {
                ...
                onError: (err) => {
                    autoGeneratedTaskIds.current.delete(nextQuizTask.task.id)

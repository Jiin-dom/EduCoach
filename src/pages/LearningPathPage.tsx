...
export default function LearningPathPage() {
    ...
    const flashcardActivityDocumentIdsToday = useMemo(() => {
        ...
        allFlashcards.forEach((card) => {
            const reviewedToday = !!card.last_reviewed_at && localDateFromTimestamp(card.last_reviewed_at) === todayLocal
            const createdToday = localDateFromTimestamp(card.created_at) === todayLocal
    ...
    const dueTodayQuizzes = useMemo(() => {
        ...
        const adaptiveQuizTasks = adaptiveTasks
            ...
            .map((task) => {
                ...
                return {
                    ...
                    href: resolvedQuiz?.id ? `/quizzes/${resolvedQuiz.id}` : "/quizzes",
                }
            })
        ...
        const dueTodayStudyTasks = adaptiveTasks
            ...
            .filter((task) => {
                if (task.type !== "flashcards") return true
                return !flashcardActivityDocumentIdsToday.has(task.documentId)
            })
            .map((task) => ({
                id: task.id,
                itemType: task.type,
    ...
    }, [documents, quizzes, adaptiveTasks, attempts, scopeFilter, matchesAdaptiveTaskScope, flashcardActivityDocumentIdsToday])
    ...
    const autoGeneratedFlashcardTaskIds = useRef<Set<string>>(new Set())
    ...
    useEffect(() => {
        const { todayLocal, completedAdaptiveDocumentIdsToday } = adaptiveQuizPolicy

        const nextFlashcardTask = adaptiveTasks
            .filter((task) => task.type === "flashcards" && !!task.scheduledDate)
            .sort((a, b) => a.scheduledDate.localeCompare(b.scheduledDate))
            .find((task) =>
                task.scheduledDate <= todayLocal &&
                !completedAdaptiveDocumentIdsToday.has(task.documentId) &&
                !flashcardActivityDocumentIdsToday.has(task.documentId) &&
                (dueFlashcardsByDocument.get(task.documentId) || 0) === 0,
            )

        if (!nextFlashcardTask) return
        if (generateAdaptiveFlashcards.isPending) return
        if (autoGeneratedFlashcardTaskIds.current.has(nextFlashcardTask.id)) return

        autoGeneratedFlashcardTaskIds.current.add(nextFlashcardTask.id)
        toast.loading("Generating adaptive flashcards...")
        generateAdaptiveFlashcards.mutate(
            {
                documentId: nextFlashcardTask.documentId,
                focusConceptIds: nextFlashcardTask.conceptIds,
            },
            {
                onSuccess: (result) => {
                    toast.dismiss()
                    if (result.createdCount > 0) {
                        toast.success(`Generated ${result.createdCount} adaptive flashcards for today.`)
                    } else {
                ...
                onError: (err) => {
                    toast.dismiss()
                    autoGeneratedFlashcardTaskIds.current.delete(nextFlashcardTask.id)
                    toast.error("Adaptive flashcard generation failed: " + (err as Error).message)
                },
            },
        )
    }, [adaptiveQuizPolicy, adaptiveTasks, dueFlashcardsByDocument, flashcardActivityDocumentIdsToday, generateAdaptiveFlashcards])

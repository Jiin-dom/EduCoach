...
-- 030: Restore adaptive-task guards lost when 028 replaced functions
--
-- Deleting a document fires triggers on user_concept_mastery / flashcards /
-- quizzes that call sync_adaptive_study_tasks_for_document(). After the
-- documents row is gone, INSERT/UPDATE on adaptive_study_tasks with that
-- document_id violates adaptive_study_tasks_document_id_fkey.
-- Migration 025 added early-exit + cleanup; 027/028 redefined the functions
-- without that guard. Restore it here (keeping 028 user_scheduled_date behavior).
-- =====================================================
...
BEGIN
    ...
    -- Document already removed (e.g. delete transaction in progress): drop tasks, do not INSERT.
    IF NOT EXISTS (
        SELECT 1
...
BEGIN
    ...
    -- Same transaction may delete the document before this runs; never recreate tasks for a missing doc.
    IF NOT EXISTS (
        SELECT 1
        FROM public.documents d
        WHERE d.id = p_document_id

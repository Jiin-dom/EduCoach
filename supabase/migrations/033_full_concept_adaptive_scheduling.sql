...
-- 033: Full-concept adaptive scheduling
--
-- Remove concept count limits so ALL document concepts
-- flow into adaptive tasks. Also update quiz title
-- pattern matching for new naming convention.
-- =====================================================
...
BEGIN
    ...
    WHERE q.user_id = p_user_id
      ...
      AND (q.title ILIKE 'Review Quiz:%'
           OR q.title ILIKE 'Adaptive:%'
           OR q.title ILIKE 'Baseline:%'
           OR q.title ILIKE 'Review:%')
    ...
    VALUES (
        ...
        v_linked_quiz_id,
        jsonb_build_object(
            'questionCount', GREATEST(10, LEAST(20, v_concept_count * 2))
        ),
        NOW()
    )

...
-- 032: Study-day aware adaptive scheduling
--
-- Keep adaptive study task generation aligned with the
-- learner's available study days and the app calendar day.
-- =====================================================

CREATE OR REPLACE FUNCTION public.study_day_id_for_date(p_date DATE)
RETURNS TEXT
LANGUAGE sql
IMMUTABLE
AS $$
...
CREATE OR REPLACE FUNCTION public.next_available_study_date(
    p_user_id UUID,
    p_start_date DATE
)
RETURNS DATE
...
BEGIN
    ...
    SELECT up.available_study_days
    ...
    FOR v_offset IN 0..13 LOOP
        v_candidate := p_start_date + v_offset;
        IF public.study_day_id_for_date(v_candidate) = ANY(v_available_days) THEN
            RETURN v_candidate;
        END IF;
    END LOOP;
...
REVOKE ALL ON FUNCTION public.next_available_study_date(UUID, DATE) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.next_available_study_date(UUID, DATE) TO authenticated, service_role;

...
-- 033: Spaced adaptive study tasks
--
-- Spread adaptive study tasks (quiz, flashcards, review)
-- across future study days to avoid clustering and enforce
...
DECLARE
    ...
    v_app_today DATE := (NOW() AT TIME ZONE 'Asia/Manila')::DATE;
    ...
    -- New variables for spaced dates
    v_quiz_date DATE;
    v_flashcards_date DATE;
    v_review_date DATE;
BEGIN
    ...
    SELECT
        ...
        COUNT(*),
        CASE
            WHEN BOOL_OR(t.has_attempt AND t.due_date <= v_app_today) THEN 'due_today'
    ...
    FROM (
        ...
        urgent AS (
            ...
            FROM candidates c
            WHERE
                c.due_date <= v_app_today
                OR c.mastery_level IN ('needs_review', 'developing')
            ORDER BY
                CASE
                    WHEN c.due_date <= v_app_today THEN 0
    ...
    IF v_scheduled_date IS NOT NULL THEN
        v_scheduled_date := public.next_available_study_date(p_user_id,
            GREATEST(v_scheduled_date, v_app_today)
        );
    END IF;
    
    -- Calculate spaced repetition dates based on reasoning
    IF v_reason = 'needs_review' THEN
        -- Weak topics: Day 1: Review, Day 2: Flashcards, Day 3: Quiz retry
        v_review_date := v_scheduled_date;
        v_flashcards_date := public.next_available_study_date(p_user_id, v_review_date + 1);
        v_quiz_date := public.next_available_study_date(p_user_id, v_flashcards_date + 1);
    ELSIF v_reason = 'developing' THEN
        -- Medium topics: Spread out further
        v_flashcards_date := v_scheduled_date;
        v_quiz_date := public.next_available_study_date(p_user_id, v_flashcards_date + 2);
        v_review_date := public.next_available_study_date(p_user_id, v_quiz_date + 2);
    ELSE
        -- due_today or fresh content: Prioritize Quiz and space out reinforcement
        v_quiz_date := v_scheduled_date;
        v_flashcards_date := public.next_available_study_date(p_user_id, v_quiz_date + 1);
        v_review_date := public.next_available_study_date(p_user_id, v_flashcards_date + 2);
    END IF;

...
-- 024: Persistent Adaptive Study Tasks
-- =====================================================

CREATE TABLE IF NOT EXISTS public.adaptive_study_tasks (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
    document_id UUID NOT NULL REFERENCES public.documents(id) ON DELETE CASCADE,
    task_key TEXT NOT NULL,
    task_type TEXT NOT NULL CHECK (task_type IN ('quiz', 'flashcards', 'review')),
    status TEXT NOT NULL CHECK (status IN ('pending_generation', 'generating', 'ready', 'archived')),
    reason TEXT CHECK (reason IN ('due_today', 'needs_review', 'developing')),
    scheduled_date DATE,
    priority_score NUMERIC(5,4) NOT NULL DEFAULT 0.0000,
    ...
    CONSTRAINT adaptive_study_tasks_user_task_key_uniq UNIQUE (user_id, task_key)
);

CREATE INDEX IF NOT EXISTS idx_adaptive_tasks_user_status_date
    ON public.adaptive_study_tasks(user_id, status, scheduled_date, priority_score DESC);

CREATE INDEX IF NOT EXISTS idx_adaptive_tasks_user_document_type
    ON public.adaptive_study_tasks(user_id, document_id, task_type);

CREATE INDEX IF NOT EXISTS idx_adaptive_tasks_linked_quiz
    ON public.adaptive_study_tasks(linked_quiz_id);

ALTER TABLE public.adaptive_study_tasks ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Users can view own adaptive study tasks" ON public.adaptive_study_tasks;
CREATE POLICY "Users can view own adaptive study tasks"
    ON public.adaptive_study_tasks FOR SELECT
    USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Service role can manage adaptive study tasks" ON public.adaptive_study_tasks;
CREATE POLICY "Service role can manage adaptive study tasks"
    ON public.adaptive_study_tasks FOR ALL
    USING (auth.role() = 'service_role')
    WITH CHECK (auth.role() = 'service_role');

DROP TRIGGER IF EXISTS update_adaptive_study_tasks_updated_at ON public.adaptive_study_tasks;
CREATE TRIGGER update_adaptive_study_tasks_updated_at
    BEFORE UPDATE ON public.adaptive_study_tasks
    FOR EACH ROW
...
CREATE OR REPLACE FUNCTION public.trg_sync_adaptive_tasks_from_mastery()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
    IF TG_OP = 'DELETE' THEN
        PERFORM public.sync_adaptive_study_tasks_for_document(OLD.user_id, OLD.document_id);
        RETURN OLD;
    END IF;

    PERFORM public.sync_adaptive_study_tasks_for_document(NEW.user_id, NEW.document_id);

    IF TG_OP = 'UPDATE'
       AND (OLD.user_id IS DISTINCT FROM NEW.user_id OR OLD.document_id IS DISTINCT FROM NEW.document_id)
...
DROP TRIGGER IF EXISTS trg_sync_adaptive_tasks_from_mastery ON public.user_concept_mastery;
CREATE TRIGGER trg_sync_adaptive_tasks_from_mastery
    AFTER INSERT OR UPDATE OR DELETE ON public.user_concept_mastery
    FOR EACH ROW
    EXECUTE FUNCTION public.trg_sync_adaptive_tasks_from_mastery();

CREATE OR REPLACE FUNCTION public.trg_sync_adaptive_tasks_from_flashcards()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
DROP TRIGGER IF EXISTS trg_sync_adaptive_tasks_from_flashcards ON public.flashcards;
CREATE TRIGGER trg_sync_adaptive_tasks_from_flashcards
    AFTER INSERT OR UPDATE OR DELETE ON public.flashcards
    FOR EACH ROW
    EXECUTE FUNCTION public.trg_sync_adaptive_tasks_from_flashcards();

CREATE OR REPLACE FUNCTION public.trg_sync_adaptive_tasks_from_quizzes()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
    v_is_review_quiz BOOLEAN := FALSE;
    v_old_is_review_quiz BOOLEAN := FALSE;
BEGIN
    IF TG_OP = 'DELETE' THEN
        v_old_is_review_quiz := COALESCE(OLD.title ILIKE 'Review Quiz:%', FALSE);
        IF v_old_is_review_quiz THEN
            PERFORM public.sync_adaptive_study_tasks_for_document(OLD.user_id, OLD.document_id);
        END IF;
        RETURN OLD;
    END IF;

    v_is_review_quiz := COALESCE(NEW.title ILIKE 'Review Quiz:%', FALSE);

    IF v_is_review_quiz THEN
        PERFORM public.sync_adaptive_study_tasks_for_document(NEW.user_id, NEW.document_id);
    END IF;
    ...
    IF TG_OP = 'UPDATE' THEN
        ...
        IF v_old_is_review_quiz
           AND (
               OLD.user_id IS DISTINCT FROM NEW.user_id
               OR OLD.document_id IS DISTINCT FROM NEW.document_id
               OR NOT v_is_review_quiz
           ) THEN
            PERFORM public.sync_adaptive_study_tasks_for_document(OLD.user_id, OLD.document_id);
        END IF;
    END IF;
...
DROP TRIGGER IF EXISTS trg_sync_adaptive_tasks_from_quizzes ON public.quizzes;
CREATE TRIGGER trg_sync_adaptive_tasks_from_quizzes
    AFTER INSERT OR UPDATE OR DELETE ON public.quizzes
    FOR EACH ROW
    EXECUTE FUNCTION public.trg_sync_adaptive_tasks_from_quizzes();
...
BEGIN
    FOR v_row IN
        SELECT DISTINCT ucm.user_id, ucm.document_id
        FROM public.user_concept_mastery ucm
        WHERE ucm.document_id IS NOT NULL
    LOOP
        PERFORM public.sync_adaptive_study_tasks_for_document(v_row.user_id, v_row.document_id);
    END LOOP;
END;
$$;

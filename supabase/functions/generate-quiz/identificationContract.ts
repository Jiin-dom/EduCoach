...
export function isIdentificationPromptContractValid(prompt: string): boolean {
    return !IDENTIFICATION_LONG_FORM_PROMPT_RE.test(prompt.trim())
}
...
export function filterInvalidIdentificationQuestions<T extends IdentificationContractQuestion>(questions: T[]): T[] {
    return questions.filter((question) => {
        ...
        return isIdentificationAnswerContractValid(question.correct_answer)
            && isIdentificationPromptContractValid(question.question_text)
    })
}
